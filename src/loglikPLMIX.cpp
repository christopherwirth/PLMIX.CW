...
/* Iteration to find the smallest non-zero value of exp(f) in cases where exp(f) is erroneously being returned as 0
 * Approximation to avoid ll being returned as -Inf
 * Added 09/01/2023 by Chris Wirth, christopher.wirth@manchester.ac.uk */
while(weights[j]*exp(f) == 0) {
  f++;  
}
